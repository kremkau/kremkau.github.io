---
title: "Mühlenaller im Oebisfelder Dämmchen"
date: 2026-08-16
slug: "wassertretstelle-oebisfelde-muehlenaller"
description: "Naturnahe Wassertret- und Spielstelle für Kinder an der Mühlenaller im Oebisfelder Dämmchen."
type: "wassertretstelle"
rubriken:
  - "wassertretstellen"  
tags:
  - Wassertreten
  - Kneipp
  - Oebisfelde
  - Mühlenaller
  - Drömling
  - Aller-Radweg
location: "Oebisfelde"
status: "geöffnet"
verification:
  date: "2026-08-216"
  method: "persönlicher Besuch"
  personally_visited: true

season:
  from: null
  until: null

water:
  type: "Naturwasser"
  source: "Mühlenaller"
  integrated_into_watercourse: true
  length_m: null
  depth_cm: null
  temperature: null

access:
  free: true
  opening_hours: "jederzeit zugänglich"
  wheelchair_accessible: null
  child_friendly: true
  children_observed: null
  handrail: true
  changing_facility: null
  toilet: null
  drinking_water: null
  dogs_allowed: null

arrival:
  address: "Lindenstraße 29, 39646 Oebisfelde"
  parking: null
  public_transport: null
  bicycle_access: true
  bicycle_route: "Aller-Radweg"
  walking_distance_m: null

visit:
  date: "2026-08-16"
  time: "14:00 Uhr"
  status_observed: "in Betrieb und genutzt"
  visitors_observed:
    - null
  photos_taken: true
  installation_photos_taken: true
  photo_shows: "Wassertretstelle an der Mühlenaller"

operator:
  name: "Stadt Oebisfelde"
  address: null
  telephone: null
  email: null
  website: null

map:
  openstreetmap: "[Standort bei OpenStreetMap](https://www.openstreetmap.org/?mlat=52.432083&mlon=10.985167#map=19/52.432083/10.985167&layers=N)" 

images:
  - "/images/20260824_Wassertretstelle-Kalbe-Kurpark.jpeg"
featured_image: "/images/20260824_Wassertretstelle-Kalbe-Kurpark.jpeg"

---

Die [Wassertretstelle liegt an der Mühlenaller in Oebisfelde](https://www.stadt-oebisfelde-weferlingen.de/de/ortschaften/oebisfelde.html), im Bereich des Dämmchens zwischen Theodor-Müller-Straße und Lessingstraße. Sie ist unmittelbar in den Lauf des Gewässers eingebunden und wird von dessen Wasser durchströmt. Die Mühlenaller selbst wird über den Landgraben mit Wasser aus der Aller gespeist.

Die Anlage wurde als Wassertret- und Spielstelle gestaltet. Großformatige Steinplatten bilden den befestigten Bereich im Wasser. Stufen und Rampen ermöglichen die Nutzung bei unterschiedlichen Wasserständen; ein umlaufendes Edelstahlgeländer bietet Halt. Ein angrenzender, ebenfalls mit Flusswasser versorgter Bereich lädt Kinder zum Spielen ein.

## Ausstattung

- Wassertretstelle mit natürlichem, fließendem Wasser der Mühlenaller
- befestigte Trittfläche aus großformatigen Steinplatten
- Stufen und Rampen für unterschiedliche Wasserstände
- umlaufendes Edelstahlgeländer mit Handlauf
- angrenzender Wasserspielbereich für Kinder
- Lage in der öffentlichen Grünanlage Dämmchen
- kostenlose Nutzung
- öffentlich zugänglich

Die Idee für die Anlage entstand 2009 in der Arbeitsgruppe Altstadt. Umgesetzt wurde sie im Zuge der Aufwertung des Oebisfelder Stadtzentrums.

## Anreise

Die Wassertretstelle befindet sich ineiner langgestreckten Grünanlage an der Altstadt parallel zum Verlauf der Mühlenaller. Der Zugang erfolgt über den Verbindungsweg zwischen Theodor-Müller-Straße und Lessingstraße.

In der Nähe treffen mit dem Aller-Radweg, dem EuroVelo 13 und dem Rundweg „Velpker Schweiz“ mehrere Radrouten zusammen. Die Radweg-Infostelle auf dem Dämmchen liegt ebenfalls direkt an der Mühlenaller. Dadurch eignet sich die Wassertretstelle gut für eine Pause während einer Wanderung oder Fahrradtour.

→ [Standort bei OpenStreetMap](https://www.openstreetmap.org/way/281488867)

## Mein Besuch

<!-- Diesen Abschnitt nach dem Vor-Ort-Termin ergänzen: Datum, Uhrzeit, Wasserstand, Zugänglichkeit, Nutzung und persönlicher Eindruck. -->

## Noch zu prüfen

Folgende Angaben sollen bei einem Besuch kontrolliert und ergänzt werden:

- aktueller Zustand und Nutzbarkeit der Anlage
- genaue Länge und Wassertiefe
- Beschaffenheit und Rutschfestigkeit des Einstiegs
- Zugänglichkeit für Menschen mit Mobilitätseinschränkungen
- Sitzgelegenheiten unmittelbar an der Anlage
- Toiletten in der näheren Umgebung
- saisonale Einschränkungen
- genaue Entfernung zum Bahnhof und zu geeigneten Parkmöglichkeiten
- aktuelles Foto mit Alternativtext und Bildunterschrift
